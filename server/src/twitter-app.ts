...
type CallbackResolveFunc = (value?: string[] | PromiseLike<string[]>) => void;
type CallbackRejectFunc = (reason: any) => void;
...
export class TwitterApp {
    ...
    constructor() {
        this.twitter = new Twitter({
            consumerKey: this.apiKey, consumerSecret: this.apiSecret,
    ...
    public getLatestTweet(username: string): Promise<string[]> {
        return this.getTweets(username, 1);
    }
    ...
    public getTweets(username: string, count: number): Promise<string[]> {
        return new Promise<string[]>((resolve, reject) => {
            ...
            this.getTweetsHelper(username, promise, results, nextCount, remaining, resolve, reject);
        });
    }

    private getTweetsHelper(username: string,
                            previousPromise: Promise<Tweets>,
                            results: string[],
                            nextCount: number,
                            remaining: number,
                            resolve: CallbackResolveFunc,
                            reject: CallbackRejectFunc) {
            previousPromise
            .then(tweets => {
                ...
                if (remaining == 0) {
                    ...
                    this.getTweetsHelper(username, this.getLatestTweetsByCount(username, nextCount, tweets.max_id), results, nextCount, remaining, resolve, reject);
                }
            })
